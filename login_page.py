...
class login_page:
    def __init__(self, root):
        self.window = root
        self.window.title("Log In PyThon")
        # Set the window size
        ...
        self.frame1.place(x=0, y=0, width=250, relheight = 1)

        label1 = Label(self.frame1, text= "Py", font=("times new roman", 40, "bold"), bg="yellow", fg="red").place(x=20,y=120)
        label2 = Label(self.frame1, text= "Thon", font=("times new roman", 40, "bold"), bg="yellow", fg="RoyalBlue1").place(x=80,y=120)
        label3 = Label(self.frame1, text= "It's all about Python Learning", font=("times new roman", 13, "bold"), bg="yellow", fg="brown4").place(x=10,y=180)

        #=============Entry Field & Buttons============

        self.frame2 = Frame(self.window, bg = "gray95")
        self.frame2.place(x=250,y=0,relwidth=1, relheight=1)

        self.frame3 = Frame(self.frame2, bg="white")
        self.frame3.place(x=0,y=0,width=500,height=500)

        self.email_label = Label(self.frame3,text="Email Address", font=("helvetica",20,"bold"),bg="white", fg="gray").place(x=10,y=40)
        self.email_entry = Entry(self.frame3,font=("times new roman",15,"bold"),bg="white",fg="gray")
        self.email_entry.place(x=10, y=80, width=300)

        self.password_label = Label(self.frame3,text="Password", font=("helvetica",20,"bold"),bg="white", fg="gray").place(x=10,y=120)
        self.password_entry = Entry(self.frame3,font=("times new roman",15,"bold"),bg="white",fg="gray",show="*")
        self.password_entry.place(x=10, y=160, width=300)

        #================Buttons===================
        self.login_button = Button(self.frame3,text="Log In",command=self.login_func,font=("times new roman",15, "bold"),bd=0,cursor="hand2",bg="blue",fg="white").place(x=60,y=200,width=200)

        self.forgotten_pass = Button(self.frame3,text="Forgotten password?",command=self.forgot_func,font=("times new roman",10, "bold"),bd=0,cursor="hand2",bg="white",fg="blue").place(x=80,y=250,width=150)

        self.create_button = Button(self.frame3,text="Create New Account",command=self.redirect_window,font=("times new roman",18, "bold"),bd=0,cursor="hand2",bg="green2",fg="white").place(x=40,y=295,width=250)
    ...
    def login_func(self):
        if self.email_entry.get()=="" or self.password_entry.get()=="":
            ...
            try:
                connection=pymysql.connect(host="host", user="user", password="non", database="database")
                ...
                if row == None:
                    ...
                    messagebox.showinfo("Success","Welcome to the PyThon family",parent=self.window)
